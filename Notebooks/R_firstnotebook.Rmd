...
```{r}
ggplot(top3, aes(x=Year, y=`Life Expectancy`, color=Country)) +
  geom_line() +
  facet_wrap(~Country)
...
```{r}
cor(gdp_le_2021$GDP_Per_Capita, gdp_le_2021$`Life Expectancy`, use='complete.obs')
```
...
```{r}
cor(gdp_le_2021$log_gdp, gdp_le_2021$`Life Expectancy`, use='complete.obs')
```
Interpretation: There is a stronger correlation the log_gdp and life expectancy.


```{r}
ggplot(gdp_le_2021, aes(x=log_gdp, y=`Life Expectancy`)) +
geom_point()
```
